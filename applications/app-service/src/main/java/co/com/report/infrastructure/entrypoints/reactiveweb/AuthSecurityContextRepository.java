...
import org.springframework.web.server.ServerSecurityContextRepository;
...
public class AuthSecurityContextRepository implements ServerSecurityContextRepository {
    ...
    public Mono<SecurityContext> load(ServerWebExchange exchange) {
        ...
        return getAuthService.getByToken(token)
            .map(auth -> {
                var loggedUser = LoggedUser.builder().email(auth.getEmail()).build();
                var authentication = new UsernamePasswordAuthenticationToken(loggedUser, null, java.util.List.of());
